...
module.exports = {
    data: new Discord.SlashCommandBuilder().setName("queue").setDescription("Muestra la cola de reproducción actual del servidor."),
    memberVoice: true,
    botVoice: true,
    sameVoice: true,
    queueNeeded: true,
    ...
    async execute(client, interaction, memberVC, botVC, queue) {
        ...
        for (let i = 0; n > i; i++) {
            const queueEmbed = new Discord.EmbedBuilder()
                ...
                .setTitle(`📃 Cola de ${interaction.guild.name} [${i + 1}/${Math.ceil(n)}]`)
        ...
        collector.on("collect", async (int) => {
            if (int.member.id !== interaction.user.id)
                return int.reply({
                    content: `Este botón solo funciona para ${interaction.user.globalName || interaction.user.username}`,
                    ephemeral: true,
                });
